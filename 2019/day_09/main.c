...
long	g_halts[] = {0, 0, 0, 0, 0};
long	g_first[] = {0, 0, 0, 0, 0};

long	g_ptr[] = {0, 0, 0, 0, 0};

long	g_phase_setting[] = {
	1, 5, 5, 5, 5
};

void	intcode(long *program)
{
	long	relative_base;
	long	reg[4];
	...
	while (op != 99)
	{
		clear_reg(reg, sizeof(reg) / sizeof(*reg));
		if (op % 10 == 1)
			operation_01(program, reg, &g_ptr[0], relative_base);
		else if (op % 10 == 2)
			operation_02(program, reg, &g_ptr[0], relative_base);
		else if (op % 10 == 3)
		{
		...
		{
			reg[1] = get_value(program, (program[g_ptr[0]] / 100) % 10, program[g_ptr[0] + 1], relative_base);
			g_output[0] = reg[1];
			printf("OUT: %ld; at %ld\n", g_output[0], g_ptr[0]);
			printf("HERE %ld %ld\n", program[g_ptr[0] + 0], program[g_ptr[0] + 1]);
			g_ptr[0] += 2;
		}
		else if (op % 10 == 5)
			operation_05(program, reg, &g_ptr[0], relative_base);
		else if (op % 10 == 6)
			operation_06(program, reg, &g_ptr[0], relative_base);
		else if (op % 10 == 7)
			operation_07(program, reg, &g_ptr[0], relative_base);
		else if (op % 10 == 8)
			operation_08(program, reg, &g_ptr[0], relative_base);
		else if (op % 10 == 9)
		{
			// reg[1] = get_value(program, (program[g_ptr[0]] / 100) % 10, c, relative_base);
			relative_base += program[g_ptr[0] + 1];
			g_ptr[0] += 2;
		}
		else
		{
			printf("Wrong instruction: OP: %ld\n", program[g_ptr[0]]);
			break ;
		}
...
int		main(void)
{
	...
	long	exam[] = {109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99};

	long	attempt[sizeof(exam) + VIRTUAL_MEMORY_SIZE];

	bzero(attempt, sizeof(attempt));
	memcpy(attempt, BOOST, sizeof(BOOST));
