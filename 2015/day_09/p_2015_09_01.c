...
/*   Updated: 2020/09/28 20:09:10 by home             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	depth_first_permute(int visited, int limit, bool *visited_list, int *visit_order, int curr_dis, int *min_dis)
{
	int		i;
	int		new_dis;

	if (visited == limit || curr_dis > *min_dis)
	{
		*min_dis = MIN(curr_dis, *min_dis);
		return ; // Possible route, and update to min_dis or branch cannot contain shortest possible route.
	}
	...
	while (i < limit)
	{
		if (visited_list[i] == false)
		{
			...
			depth_first_permute(visited + 1, limit, visited_list, visit_order, new_dis, min_dis);
...
int		main(void)
{
	...
	depth_first_permute(0, cities, visited_list, visit_order, 0, &min_dis);
